...
const FunctionalCalendar = ({
    ...
    const isSelectedDay = (day) => {
        ...
        if (currentDay === start) {
            return "bg-accent text-white hover:bg-blue-800 rounded-l-xl font-bold";
        }
        if (currentDay === end) {
            return "bg-accent text-white hover:bg-blue-800 rounded-r-lg font-bold";
        }
        if (isBefore(day, endDate) && isAfter(day, startDate)) {
            return "bg-gray-100 text-accent hover:bg-gray-200 font-bold";
        }
        if (!isSameMonth(day, buildDate)) {
            return "text-gray-300 hover:bg-gray-100 rounded-full";
        }
        if (currentDay === now) {
            return "text-accent underline rounded-full hover:bg-gray-100";
        } else {
    ...
    const isOnOutside = (day) => {
        ...
        const isStartWeek = isSameWeek(day, startDate);
        const isEndWeek = isSameWeek(day, endDate);

        if (isSat && isSameWeek(subDays(endDate, 7), day)) {
            if (isStartWeek) {
                return "rounded-r-xl";
            }
            return "rounded-br-xl";
        }
        if (isSun && isSameWeek(addDays(startDate, 7), day)) {
            if (isEndWeek) {
                return "rounded-l-xl";
            }
            return "rounded-tl-xl";
        }
        if ((isSun && isStartWeek) || (isSun && isEndWeek)) {
            return "rounded-bl-xl";
        }
        if ((isSat && isStartWeek) || (isSat && isEndWeek)) {
                ...
                <div className="">
                    <div className=" shadow-xl flex flex-col bg-white rounded-xl">
                        <div>
                            <div className="flex justify-between items-center mt-5 font-bold text-lg">
                                <ChevronLeftIcon
                                    ...
                                    className="h-9 w-9 md:ml-8 ml-3 cursor-pointer text-accent hover:bg-gray-100 p-2 rounded-full"
                                />
                                <div>
                                ...
                                <ChevronRightIcon
                                    onClick={nextMonth}
                                    className="h-9 w-9 md:mr-8 mr-3 cursor-pointer text-accent hover:bg-gray-100 p-2 rounded-full"
                                />
                            </div>
                        ...
                        <div className="flex justify-center items-center mb-6">
                            ...
                            <ArrowRightIcon className="h-8 w-11 px-3 py-1 text-accent" />
